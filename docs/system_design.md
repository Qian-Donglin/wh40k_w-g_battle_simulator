...
| 対集団武器（Blast/Flamer等の範囲攻撃）のMagnitude削り | **解決済み**：2層構造（幾何判定＋Horde占有領域との重なり）。基準密度 a=4m²。対Hordeは図形が重なっていれば `n_hit = min(N, Blast Size対象数)` の二値判定、対個体は座標で幾何判定。対象数は攻撃全体の総枠。範囲攻撃は常時Spread適用外（`docs/area_attack_design.md` 参照） | 解決済み |
| 戦場の座標系 | **解決済み**：2D・連続座標（メートル単位、グリッドなし）。個体は点、Hordeは円（`r = 1.128√N`、向きを持たない）。Z軸は持たない（`docs/simulator_app_design.md` 参照） | 解決済み |
